...
Write-Utf8NoBom "$root\Domain\NeedStatus.cs" $needStatus
Write-Utf8NoBom "$root\Endpoints\OfferEndpoints.cs" $offerEndpoints

$providerPanelPath = Join-Path $root "Endpoints\ProviderPanelEndpoints.cs"
if (Test-Path $providerPanelPath) {
    $providerPanel = Get-Content -Raw -Encoding UTF8 $providerPanelPath

    $providerPanel = $providerPanel.Replace(
        'x.Status != NeedStatus.Completed &&',
        'x.Status != NeedStatus.Completed &&' + "`r`n" +
        '                    x.Status != NeedStatus.Cancelled &&'
    )

    Write-Utf8NoBom $providerPanelPath $providerPanel
}

Write-Host ""
Write-Host "Talep iptali ve teklif geri cekme backend altyapisi hazirlandi." -ForegroundColor Green
Write-Host ""
Write-Host "Yeni durum:" -ForegroundColor Cyan
Write-Host "  NeedStatus.Cancelled"
Write-Host ""
Write-Host "Yeni endpointler:" -ForegroundColor Cyan
Write-Host "  POST /api/my-needs/{needId}/cancel"
Write-Host "  POST /api/provider-panel/offers/{offerId}/withdraw"
Write-Host ""
Write-Host "Kurallar:" -ForegroundColor Cyan
Write-Host "  Sonuclanmis talep iptal edilemez"
Write-Host "  Iptal edilen talebe yeni teklif verilemez"
Write-Host "  Talep iptalinde bekleyen teklifler otomatik geri cekilir"
Write-Host "  Yalnizca bekleyen teklif geri cekilebilir"
Write-Host "  Geri cekilen teklif ayni isletme tarafindan tekrar verilemez"
Write-Host ""
Write-Host "Not: NeedStatus veritabaninda string saklandigi icin migration gerekmiyor." -ForegroundColor DarkYellow
Write-Host ""
