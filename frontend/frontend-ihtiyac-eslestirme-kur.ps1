...
$page = Join-Path $root "src\app\ihtiyac-olustur\page.tsx"

if (-not (Test-Path $page)) {
    throw "Dosya bulunamadi: $page"
}
...
export default function NeedCreatePage() {
  ...
  const services = useMemo(() => {
    return (
      categories.find((item) => item.slug === categorySlug)?.services ?? []
    );
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const matchResponse = await fetch(
        `${API_BASE}/api/needs/${created.id}/matches`,
      );
...
Write-Host ""
Write-Host "Ihtiyac olusturma sayfasi yeni eslestirme API'sine uyarlandi." -ForegroundColor Green
Write-Host "Degistirilen dosya:" -ForegroundColor Cyan
Write-Host "  src\app\ihtiyac-olustur\page.tsx"
Write-Host ""
Write-Host "Yeni form artik kategori + hizmet + il + ilce sluglarini backend'e gonderiyor." -ForegroundColor Yellow
Write-Host "Talep olustuktan sonra uygun yayindaki isletmeler otomatik getiriliyor." -ForegroundColor Yellow
Write-Host ""
